...
const LINE_FILTERS = (process.env.LINE_FILTERS ?? "").split("|");
const ACTIONS = (process.env.ACTIONS ?? "").split("|");
...
async function getLineToHref() {
  ...
  for (let line of content.split("\n")) {
    ...
    if (!LINE_FILTERS.some((filter) => line.includes(filter))) continue;
...
logger.info({ LINE_FILTERS, ACTIONS });
